<template>
  <div class="mr-page">
    <div class="page-head">
      <div>
        <h2 class="page-title">{{ t('mr.title') }}</h2>
        <p class="page-desc">{{ t('mr.desc') }}</p>
      </div>
      <div class="tab-bar">
        <button :class="['tab-btn', activeTab==='pending'?'active':'']" @click="activeTab='pending'">
          {{ t('mr.tabPending') }} <span class="tab-badge">{{ pendingReports.length }}</span>
        </button>
        <button :class="['tab-btn', activeTab==='archived'?'active':'']" @click="activeTab='archived'">
          {{ t('mr.tabArchived') }} <span class="tab-badge">{{ archivedReports.length }}</span>
        </button>
        <button :class="['tab-btn', activeTab==='all'?'active':'']" @click="activeTab='all'">
          {{ t('mr.tabAll') }} <span class="tab-badge">{{ reports.length }}</span>
        </button>
      </div>
    </div>

    <div class="stat-row">
      <div class="stat-card"><div class="stat-icon" style="background:#eef2ff;color:#2563eb"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></div><div><span class="stat-val">{{ reports.length }}</span><span class="stat-lbl">{{ t('mr.totalReports') }}</span></div></div>
      <div class="stat-card"><div class="stat-icon" style="background:#fef3c7;color:#d97706"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 20V10"/><path d="M18 20V4"/><path d="M6 20v-6"/></svg></div><div><span class="stat-val">{{ pendingReports.length }}</span><span class="stat-lbl">{{ t('mr.pendingReview') }}</span></div></div>
      <div class="stat-card"><div class="stat-icon" style="background:#dcfce7;color:#16a34a"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="20 6 9 17 4 12"/></svg></div><div><span class="stat-val">{{ archivedReports.length }}</span><span class="stat-lbl">{{ t('mr.archived') }}</span></div></div>
      <div class="stat-card"><div class="stat-icon" style="background:#fce7f3;color:#e11d48"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div><div><span class="stat-val">{{ thisMonth }}</span><span class="stat-lbl">{{ t('mr.newThisMonth') }}</span></div></div>
    </div>
    ...
    <div class="content-card">
      <div class="card-head">
        <div class="card-head-left">
          ...
          {{ activeTab === 'pending' ? t('mr.pendingReports') : activeTab === 'archived' ? t('mr.archivedReports') : t('mr.allReports') }}
        </div>
        <div class="card-head-right">
          ...
          <span class="toolbar-info">{{ displayReports.length }} {{ t('mr.records') }}</span>
        </div>
      </div>
      ...
      <div class="report-list" v-loading="loading">
        <div v-for="r in displayReports" :key="r.id" class="report-item">
          <div class="ri-left">
            <div class="ri-code">{{ r.reportCode || t('mr.workOrderPrefix') + r.id }}</div>
            <div class="ri-info">
              <span class="ri-road">{{ getWorkOrderTitle(r.workOrderId) }}</span>
              <span class="ri-meta">{{ r.executor }} · {{ r.materials || t('mr.noMaterials') }} · {{ formatDate(r.finishedAt) }}</span>
            </div>
          </div>
          <div class="ri-center">
          ...
          <div class="ri-right">
            <button class="action-btn" @click="viewDetail(r)">{{ t('mr.detail') }}</button>
            <button v-if="!isArchived(r)" class="action-btn action-approve" @click="approveReport(r)" :disabled="reviewing">{{ t('mr.approve') }}</button>
            <button v-if="!isArchived(r)" class="action-btn action-reject" @click="rejectReport(r)" :disabled="reviewing">{{ t('mr.reject') }}</button>
          </div>
        </div>
    ...
    <div v-if="showDetail" class="modal-overlay" @click.self="showDetail=false">
      <div class="modal-card">
        <div class="modal-head">
          <span>{{ t('mr.reportDetail') }}{{ detailTarget?.reportCode || t('mr.workOrderPrefix') + detailTarget?.id }}</span>
          <button class="modal-close" @click="showDetail=false">✕</button>
        </div>
        <div class="modal-body" v-loading="detailLoading">
          <template v-if="detailTarget">
            <div class="detail-grid">
              <div class="detail-item"><label>{{ t('mr.reportCode') }}</label><span>{{ detailTarget.reportCode || '--' }}</span></div>
              <div class="detail-item"><label>{{ t('mr.status') }}</label><span :style="{color: reportStatusColor(detailTarget.status), fontWeight:600}">{{ reportStatusLabel(detailTarget.status) }}</span></div>
              <div class="detail-item"><label>{{ t('mr.relatedOrder') }}</label><span>#{{ detailTarget.workOrderId }} {{ getWorkOrderTitle(detailTarget.workOrderId) }}</span></div>
              <div class="detail-item"><label>{{ t('mr.orderStatus') }}</label><span>{{ getWorkOrderStatusText(detailTarget.workOrderId) }}</span></div>
              <div class="detail-item"><label>{{ t('mr.executor') }}</label><span>{{ detailTarget.executor }}</span></div>
              <div class="detail-item"><label>{{ t('mr.finishedAt') }}</label><span>{{ formatDate(detailTarget.finishedAt) }}</span></div>
        ...
        <div class="modal-foot">
          <button class="btn-ghost" @click="showDetail=false">{{ t('mr.close') }}</button>
          <button v-if="detailTarget && !isArchived(detailTarget)" class="btn-success" @click="approveReport(detailTarget)" :disabled="reviewing">{{ t('mr.approveBtn') }}</button>
          <button v-if="detailTarget && !isArchived(detailTarget)" class="btn-danger" @click="rejectReport(detailTarget)" :disabled="reviewing">{{ t('mr.rejectBtn') }}</button>
        </div>
      </div>
    </div>
  </div>
...
function getWorkOrderStatusText(workOrderId: number): string {
  ...
  const map: Record<string, string> = {
    PENDING_ASSIGNMENT: t('wo.statusPendingAssignment'),
    ASSIGNED: t('wo.statusAssigned'),
    IN_PROGRESS: t('wo.statusInProgress'),
    COMPLETED: t('wo.statusCompleted'),
    CLOSED: t('wo.statusClosed'),
    CANCELLED: t('wo.statusCancelled'),
  }
...
function reportStatusLabel(s?: string) {
  return ({
    PENDING: t('mr.statusPending'),
    DEPT_APPROVED: t('mr.statusDeptApproved'),
    APPROVED: t('mr.statusApproved'),
...
async function loadData() {
  ...
  try {
    ...
    ElMessage.error(e?.response?.data?.message || t('mr.loadFailed'))
  } finally {
    loading.value = false
  }
}
...
async function viewDetail(r: MaintenanceReportResponse) {
  ...
  try {
    ...
    ElMessage.error(e?.response?.data?.message || t('mr.detailFailed'))
  } finally {
...
async function approveReport(r: MaintenanceReportResponse) {
  try {
    await ElMessageBox.confirm(
      t('mr.approveConfirm', { code: r.reportCode || ('#' + r.id), orderId: r.workOrderId }),
      t('mr.approveTitle'),
      { type: "warning", confirmButtonText: t('mr.approveText'), cancelButtonText: t('common.cancel') }
    )
  } catch {
    return // 用户取消
  }
  ...
  try {
    ...
    ElMessage.success(t('mr.approveSuccess'))
    // 更新本地报告状态
    r.status = "APPROVED"
    // 如果详情弹窗打开，也更新
    ...
    ElMessage.error(e?.response?.data?.message || t('mr.approveFailed'))
  } finally {
...
async function rejectReport(r: MaintenanceReportResponse) {
  let reason = ""
  try {
    const result = await ElMessageBox.prompt(t('mr.rejectPrompt'), t('mr.rejectTitle'), {
      inputPlaceholder: t('mr.rejectPlaceholder'),
      inputValidator: (val: string) => (val && val.trim().length > 0) || t('mr.rejectValidator'),
    })
    reason = result.value
  } catch {
    return // 用户取消
  ...
  try {
    ...
    ElMessage.success(t('mr.rejectSuccess'))
    // 更新本地报告状态
    r.status = "REJECTED"
    if (detailTarget.value?.id === r.id) {
    ...
    ElMessage.error(e?.response?.data?.message || t('mr.rejectFailed'))
  } finally {
    reviewing.value = false
  }
}
