  ...
  <div class="dc">
    <div class="dc-top">
      <div class="dc-top-left">
        <h2 class="dc-title">{{ t('dc.title') }}</h2>
        <span class="dc-subtitle">{{ t('dc.subtitle') }}</span>
      </div>
    </div>
    ...
    <div class="dc-grid">
      ...
      <div class="card form-card">
        <div class="card-head">
          <span class="card-title">{{ t('dc.cardTitle') }}</span>
        </div>
        <div class="card-body card-body-form">
          <div class="form-group">
            <label class="form-label">{{ t('dc.dataSourceType') }}</label>
            <div class="select-wrap">
              <select v-model="form.dataSourceType" class="form-select">
                <option value="MANUAL_IMAGE">{{ t('dc.imageDetection') }}</option>
                <option value="MANUAL_VIDEO">{{ t('dc.videoDetection') }}</option>
              </select>
          ...
          <div class="form-group">
            <label class="form-label">{{ t('dc.file') }}</label>
            <div class="upload-zone" @drop.prevent="handleDrop" @dragover.prevent @click="openFileInput">
              <input ref="fileInputRef" type="file" hidden @change="e => handleFileInput(e.target as HTMLInputElement)" accept="image/*,video/*" />
              ...
              <template v-else>
                ...
                <div class="uz-text">{{ t('dc.uploadText') }}</div>
                <div class="uz-hint">{{ t('dc.uploadHint') }}</div>
              </template>
            </div>
          ...
          <div class="form-group">
            <label class="form-label">
              {{ t('dc.location') }}
              <span v-if="form.location" class="loc-badge">{{ t('dc.located') }}</span>
            </label>
            <div class="loc-row">
              <input v-model="form.location" class="form-input" :placeholder="t('dc.locationPlaceholder')" readonly @click="focusMap" />
              <button v-if="form.location" class="loc-clear" @click="form.location='';selectedPos=null">{{ t('dc.clear') }}</button>
            </div>
          </div>

          <div class="form-group">
            <label class="form-label">{{ t('dc.remark') }}</label>
            <textarea v-model="form.remark" class="form-textarea" rows="3" :placeholder="t('dc.remarkPlaceholder')"></textarea>
          </div>

          <button class="form-btn" @click="submitTask" :disabled="submitting">
            <span v-if="submitting" class="btn-loader"></span>
            <span v-if="submitting">{{ submitStatus === 'uploading' ? t('dc.uploading') : submitStatus === 'detecting' ? t('dc.detectingBtn') : t('dc.submitting') }}</span>
            <span v-else>{{ t('dc.submit') }}</span>
          </button>
          <div v-if="submitting && submitStatus === 'detecting'" class="detect-progress-tip">
            <span class="detect-tip-dot"></span>
            {{ t('dc.detectingTip') }}
          </div>
        </div>
      </div>
      ...
      <div class="card map-card">
        <div class="map-search-bar">
          <input v-model="searchKeyword" class="map-search-input" :placeholder="t('dc.searchPlaceholder')" @keyup.enter="doSearch" />
          <button class="map-search-btn" @click="doSearch">{{ t('dc.searchBtn') }}</button>
        </div>
        <div id="collectionMap" class="map-container"></div>
        <div v-if="selectedPos" class="map-pos-info">
          <span>{{ t('dc.lngLat', { lng: selectedPos.lng.toFixed(6), lat: selectedPos.lat.toFixed(6) }) }}</span>
    ...
    <div class="card tasks-card" style="margin-top:20px">
      <div class="card-head">
        <span class="card-title">{{ t('dc.cardTitle') }}</span>
        <span v-if="tasks.length" class="card-count">{{ t('dc.totalItems', { count: tasks.length }) }}</span>
      </div>
      <div class="card-body card-body-nopad">
        <div class="act-header">
          <span class="act-cell act-id">{{ t('dc.taskCode') }}</span>
          <span class="act-cell act-loc">{{ t('dc.taskLocation') }}</span>
          <span class="act-cell act-st">{{ t('dc.taskStatus') }}</span>
          <span class="act-cell act-time">{{ t('dc.taskTime') }}</span><span class="act-cell act-op">{{ t('dc.taskAction') }}</span>
        </div>
        <div v-if="loading" class="act-loading"><div class="loader"></div></div>
        <div v-for="task in tasks" :key="task.id" class="act-row">
          <span class="act-cell act-id act-code">{{ task.taskCode }}</span>
          <span class="act-cell act-loc">{{ task.location }}</span>
          <span class="act-cell act-st"><span :class="['st-badge', stCls(task.status)]">{{ stLbl(task.status) }}</span></span>
          <span class="act-cell act-time">{{ task.createdAt }}</span><span class="act-cell act-op"><button v-if="task.status==='COMPLETED'" class="view-btn" @click="viewResult(task.id)">{{ t('dc.viewResult') }}</button><span v-else class="op-na">--</span></span>
        </div>
        <div v-if="!loading && tasks.length === 0" class="act-empty">{{ t('dc.noRecords') }}</div>
      </div>
    </div>
    ...
    <div v-if="showResultModal" class="modal-overlay" @click.self="showResultModal=false">
      <div class="modal-card">
        <div class="modal-head">
          <span>{{ t('dc.resultTitle') }}{{ resultTask?.taskCode || resultTask?.id }} 
        ...
        <div class="modal-body">
          <div v-if="resultLoading" class="modal-loading">{{ t('dc.loading') }}</div>
          <div v-else-if="resultData" class="result-content">
            ...
            <div v-if="resultImageUrl" class="res-file-preview">
              ...
              <div class="res-file-name">{{ t('dc.diseaseItems', { count: resultData.items?.length || 0 }) }}</div>
            </div>
              ...
              <div v-for="(item, i) in resultData.items" :key="i" class="result-item">
                ...
                <div class="ri-info">
                  ...
                  <div class="ri-detail-row">
                    <span class="ri-detail-label">{{ t('dc.damageType') }}</span>                <span class="ri-detail-val">{{ damageTypeDesc(item.damageType) }}</span>
                  </div>
                  <div class="ri-detail-row">
                    <span class="ri-detail-label">{{ t('dc.severityLevel') }}</span>                <span class="ri-detail-val">{{ severityDesc(item.severityLevel) }}</span>
                  </div>
                  <div class="ri-detail-row">
                    <span class="ri-detail-label">{{ t('dc.confidence') }}</span>                <span class="ri-detail-val"><span class="conf-bar-wrap"><span class="conf-bar" :style="{width: (item.confidence*100)+'%'}"></span></span> {{ (item.confidence*100).toFixed(0) }}%</span>
                  </div>
            ...
            <div v-if="canDispatch(resultData)" class="res-actions">
              <button class="btn-dispatch" @click="handleDispatch(resultTask?.id, resultData)">{{ t('dc.createOrder') }}</button>
            </div>
          </div>
          <div v-else class="modal-loading">{{ t('dc.resultLoading') }}</div>
        </div>
        <div class="modal-foot">
          <button class="btn-ghost" @click="showResultModal=false">{{ t('dc.close') }}</button>
        </div>
      </div>
    </div>
  </div>
...
function doSearch() {
  ...
  placeSearch.search(searchKeyword.value.trim(), (status: string, result: any) => {
    if (status === "complete" && result.poiList?.pois?.length) {
      ...
      ElMessage.warning(t('dc.mapSearchWarning'))
    }
  })
}
...
// 提交状态跟踪
const submitStatus = ref("") // '' | 'uploading' | 'detecting'

async function submitTask() {
  if (!form.location) { ElMessage.warning(t('dc.locationRequired')); return }
  if (!uploadedFile.value) { ElMessage.warning(t('dc.fileRequired')); return }
  submitting.value = true
  const isVideo = form.dataSourceType === "MANUAL_VIDEO"
  try {
    // 提示开始上传
    submitStatus.value = "uploading"
    ElMessage.info(isVideo ? t('dc.uploadingVideo') : t('dc.uploadingImage'))
    ...
    ElMessage.success(t('dc.submitSuccess'))

    // 上传成功后才清空表单（避免上传失败时文件预览消失）
    form.fileName = ""; form.fileUrl = ""; form.remark = ""; uploadedFile.value = null

    if (taskId) {
      submitStatus.value = "detecting"
      if (isVideo) {
        ElMessage.info(t('dc.detectingVideo'))
      }
      startPolling(taskId, isVideo)
    }
  } catch (e: any) {
    submitStatus.value = ""
    if (e?.code === "ECONNABORTED") {
      ElMessage.error(t('dc.uploadTimeout'))
    } else {
      ElMessage.error(t('dc.submitFailed') + (e?.response?.data?.message || t('dc.unknownError')))
    }
  }
  finally { submitting.value = false }
}

function startPolling(taskId: number, isVideo: boolean = false) {
  polling.value = true; let attempts = 0
  // 视频检测轮询：120次 × 3秒 = 6分钟；图片检测轮询：15次 × 2秒 = 30秒
  const maxAttempts = isVideo ? 120 : 15
  ...
  pollTimer = setInterval(async () => {
    ...
    try {
      ...
      if (updated?.status === "COMPLETED") {
        stopPolling()
        submitStatus.value = ""
        ElMessage.success(t('dc.detectComplete'))
        viewResult(taskId)
      } else if (updated?.status === "FAILED") {
        stopPolling()
        submitStatus.value = ""
        ElMessage.error(t('dc.detectFailed'))
      } else if (attempts >= maxAttempts) {
        stopPolling()
        submitStatus.value = ""
        ElMessage.info(isVideo ? t('dc.detectTimeoutVideo') : t('dc.detectTimeout'))
      }
      // 每10次轮询打印进度
      if (isVideo && attempts % 10 === 0 && attempts < maxAttempts) {
        ElMessage.info(`${t('dc.detecting')} (${attempts * intervalMs / 1000}s)`)
      }
    } catch { if (attempts >= maxAttempts) { stopPolling(); submitStatus.value = "" } }
  }, intervalMs)
}
...
function stLbl(s: string) { return ({ PENDING: t('dc.statusPending'), PROCESSING: t('dc.statusProcessing'), COMPLETED: t('dc.completed'), FAILED: t('dc.failed') })[s] || s }
...
function damageTypeLabel(tp: string) { return ({ CRACK: t('dc.crack'), POTHOLE: t('dc.pothole'), MARKING_DAMAGE: t('dc.markingDamage'), ROAD_SPILL: t('dc.roadSpill') })[tp] || tp || t('dc.unknown') }
function damageTypeDesc(tp: string) { return ({ CRACK: t('dc.crackDesc'), POTHOLE: t('dc.potholeDesc'), MARKING_DAMAGE: t('dc.markingDesc'), ROAD_SPILL: t('dc.spillDesc') })[tp] || "" }
function severityLabel(s: string) { return ({ HIGH: t('dc.severityHigh'), MEDIUM: t('dc.severityMedium'), LOW: t('dc.severityLow') })[s] || s }
function severityDesc(s: string) { return ({ HIGH: t('dc.severityHighDesc'), MEDIUM: t('dc.severityMediumDesc'), LOW: t('dc.severityLowDesc') })[s] || "" }
...
async function handleDispatch(taskId: number, data: any) {
  ElMessageBox.confirm(t('dc.dispatchConfirm'), t('dc.dispatchTitle'), { confirmButtonText: t('dc.dispatchBtn'), cancelButtonText: t('dc.dispatchCancel'), type: "warning" })
    .then(async () => {
      try {
        await workOrderApi.create({ detectionTaskId: taskId })
        ElMessage.success(t('dc.dispatchSuccess'))
        window.dispatchEvent(new CustomEvent("data-updated"))
      } catch { ElMessage.error(t('dc.dispatchFailed')) }
    }).catch(() => {})
}
...
.detect-progress-tip { display: flex; align-items: center; gap: 6px; padding: 8px 12px; background: #eff6ff; border: 1px solid #dbeafe; border-radius: 8px; font-size: 12px; color: #2563eb; margin-top: 6px; }
.detect-tip-dot { width: 6px; height: 6px; background: #2563eb; border-radius: 50%; animation: pulse 1.2s ease-in-out infinite; }
