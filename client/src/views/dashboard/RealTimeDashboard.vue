  ...
  <div class="dash">
    ...
    <div class="dash-top">
      <div class="dash-top-left">
        <h1 class="dash-title">{{ t('dash.title') }}</h1>
        <span class="dash-date">{{ dateStr }}</span>
      </div>
      <div class="dash-top-right">
        <span class="dash-dot" :class="{ 'dot-pulse': polling }"></span>
        <span class="dash-status">{{ polling ? t('dash.updating') : t('dash.normal') }}</span>
    ...
    <div class="dash-grid">
      <div class="card card-wide">
        <div class="card-head">
          <span class="card-title">{{ t('dash.detectionTrend') }}</span>
          <div class="card-tabs">
      ...
      <div class="card card-narrow">
        <div class="card-head">
          <span class="card-title">{{ t('dash.crackTypeDist') }}</span>
        </div>
        <div class="card-body">
    ...
    <div class="dash-grid dash-grid-2">
      <div class="card card-half">
        <div class="card-head">
          <span class="card-title">{{ t('dash.severityDist') }}</span>
        </div>
        <div class="card-body">
      ...
      <div class="card card-half">
        <div class="card-head">
          <span class="card-title">{{ t('dash.mapTrend') }}</span>
        </div>
        <div class="card-body">
    ...
    <div class="card">
      <div class="card-head">
        <div class="card-head-left">
          <span class="card-title">{{ t('dash.recentActivity') }}</span>
          <div class="act-tabs">
            <button :class="['at-tab', { active: actTab === 'detection' }]" @click="actTab = 'detection'">{{ t('dash.detectionTasks') }}</button>
            <button :class="['at-tab', { active: actTab === 'alert' }]" @click="actTab = 'alert'">{{ t('dash.alertEvents') }}</button>
          </div>
        </div>
        <button class="card-link" @click="$router.push('/detection-results')">{{ t('dash.viewAll') }}</button>
      </div>
      <div class="card-body card-body-nopad">
        <!-- Detection Tab -->
        <template v-if="actTab === 'detection'">
          <div class="act-header">
            <span class="act-cell act-id">{{ t('dash.id') }}</span>
            <span class="act-cell act-loc">{{ t('dash.location') }}</span>
            <span class="act-cell act-src">{{ t('dash.source') }}</span>
            <span class="act-cell act-st">{{ t('common.status') }}</span>
            <span class="act-cell act-time">{{ t('dash.time') }}</span>
          </div>
          ...
          <div v-if="!loading && recentTasks.length === 0" class="act-empty">{{ t('dash.noDetection') }}</div>
        </template>
        <!-- Alert Tab -->
        <template v-if="actTab === 'alert'">
          <div class="act-header">
            <span class="act-cell act-id">Marker ID</span>
            <span class="act-cell act-loc">{{ t('dash.location') }}</span>
            <span class="act-cell act-src">{{ t('ds.diseaseType') }}</span>
            <span class="act-cell act-st">{{ t('wo.level') }}</span>
            <span class="act-cell act-time">{{ t('common.status') }}</span>
          </div>
          ...
          <div v-if="!loading && alertList.length === 0" class="act-empty">{{ t('dash.noAlert') }}</div>
        </template>
      </div>
    </div>
  </div>
...
const tabs = computed(() => [
  { key: "week", label: t("dash.week"), days: 7 },
  { key: "month", label: t("dash.month"), days: 30 },
])

// --- KPI Cards ---
const kpis = computed(() => [
  { label: t("dash.roads"), value: kpiVals.value[0], delta: kpiDeltas.value[0], bg: "#eef2ff", color: "#4361ee", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="3"/><path d="M12 1v6m0 10v6m11-11h-6M7 12H1"/></svg>' },
  { label: t("dash.cracks"), value: kpiVals.value[1], delta: kpiDeltas.value[1], bg: "#fce7f3", color: "#e11d48", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 20V10M18 20V4M6 20v-6"/></svg>' },
  { label: t("dash.todayDetection"), value: kpiVals.value[2], delta: kpiDeltas.value[2], bg: "#fef3c7", color: "#d97706", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>' },
  { label: t("dash.pendingAlerts"), value: kpiVals.value[3], delta: kpiDeltas.value[3], bg: "#fef2f2", color: "#dc2626", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>' },
  { label: t("dash.totalOrders"), value: kpiVals.value[4], delta: kpiDeltas.value[4], bg: "#f1f5f9", color: "#475569", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>' },
  { label: t("dash.totalMarkers"), value: kpiVals.value[5], delta: kpiDeltas.value[5], bg: "#f0fdf4", color: "#16a34a", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>' },
])
const kpiVals = ref(["—", "—", "—", "—", "—", "—"])
const kpiDeltas = ref([0, 0, 0, 0, 0, 0])

// --- Map Stats Cards ---
const mapStatsCards = computed(() => {
  const m = mapStats.value
  return [
    { label: t("dash.totalMarkers"), value: m?.totalMarkers ?? 0, bg: "#eef2ff", color: "#4361ee", icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>' },
    { label: t("dash.newMarkers"), value: m?.newMarkers ?? 0, bg: "#f0fdf4", color: "#16a34a", icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>' },
    { label: t("dash.repaired"), value: m?.repairedCount ?? 0, bg: "#dcfce7", color: "#15803d", icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="20 6 9 17 4 12"/></svg>' },
    { label: t("dash.pendingRepair"), value: m?.pendingRepair ?? 0, bg: "#fef3c7", color: "#d97706", icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>' },
    { label: t("dash.severe"), value: m?.highSeverityCount ?? 0, bg: "#fef2f2", color: "#dc2626", icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>' },
    { label: t("dash.medium"), value: m?.mediumSeverityCount ?? 0, bg: "#fffbeb", color: "#d97706", icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>' },
  ]
})

const actions = computed(() => [
  { label: t("dash.actDataScreen"), desc: t("dash.actDataScreenDesc"), path: "/data-screen", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="21" x2="9" y2="9"/></svg>' },
  { label: t("dash.actAiAssistant"), desc: t("dash.actAiAssistantDesc"), path: "/ai-agent", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>' },
  { label: t("dash.actWorkOrders"), desc: t("dash.actWorkOrdersDesc"), path: "/work-orders", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>' },
  { label: t("dash.actStatistics"), desc: t("dash.actStatisticsDesc"), path: "/data-statistics", icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 3v18h18"/><path d="M7 14l4-4 4 4 5-5"/></svg>' },
])

// --- Helpers ---
function srcCls(s: string) { return ({ DRONE_VIDEO: "s-drone", MANUAL_IMAGE: "s-img", MANUAL_VIDEO: "s-vid", CROWD_SOURCE: "s-crowd" } as any)[s] || "" }
function srcLbl(s: string) { return ({ DRONE_VIDEO: t("source.drone"), MANUAL_IMAGE: t("source.photo"), MANUAL_VIDEO: t("source.video"), CROWD_SOURCE: t("source.crowd") } as any)[s] || s }
function stCls(s: string) { return ({ PENDING: "st-wait", PROCESSING: "st-run", COMPLETED: "st-ok", FAILED: "st-fail" } as any)[s] || "" }
function stLbl(s: string) { return ({ PENDING: t("status.pending"), PROCESSING: t("status.inProgress"), COMPLETED: t("status.completed"), FAILED: t("status.failed") } as any)[s] || s }
function formatTime(t: string) { return t ? t.replace("T", " ").substring(0, 16) : "--" }
function damageTypeLabel(tStr: string) { return ({ CRACK: t("damage.crack"), TRANSVERSE_CRACK: t("damage.transverseCrack"), LONGITUDINAL_CRACK: t("damage.longitudinalCrack"), NET_CRACK: t("damage.netCrack"), POTHOLE: t("damage.pothole"), MARKING_DAMAGE: t("damage.markingDamage"), ROAD_SPILL: t("damage.roadSpill"), UNKNOWN: t("damage.unknown") } as any)[tStr] || tStr || "--" }
function sevCls(s: string) { return ({ HIGH: "st-fail", MEDIUM: "st-run", LOW: "st-ok" } as any)[s] || "st-wait" }
function sevLbl(s: string) { return ({ HIGH: t("severity.high"), MEDIUM: t("severity.medium"), LOW: t("severity.low") } as any)[s] || s }
function statusCls(s: string) { return ({ PENDING: "st-wait", REPAIRED: "st-ok", NEW: "st-run" } as any)[s] || "st-wait" }
function statusLbl(s: string) { return ({ PENDING: t("status.pending"), REPAIRED: t("status.repaired"), NEW: t("status.new") } as any)[s] || s }
...
async function loadDashboard() {
  try {
    ...
    if (d) {
      kpiVals.value[0] = String(d.monitoredRoads ?? 0)
      kpiVals.value[1] = String(d.totalCracksDetected ?? 0)
      kpiVals.value[2] = String(d.detectionToday ?? 0)
      kpiVals.value[3] = String(d.pendingAlerts ?? 0)
      kpiVals.value[4] = String(d.totalWorkOrders ?? 0)
    }
...
async function loadMapStats() {
  try {
    ...
    if (mapStats.value) {
      kpiVals.value[5] = String(mapStats.value.totalMarkers ?? 0)
    }
...
async function loadPie() {
  ...
  try {
    ...
    if (pieChart) {
      ...
      const typeLabels: Record<string, string> = { CRACK: t("damage.crack"), TRANSVERSE_CRACK: t("damage.transverseCrack"), LONGITUDINAL_CRACK: t("damage.longitudinalCrack"), NET_CRACK: t("damage.netCrack"), POTHOLE: t("damage.pothole"), MARKING_DAMAGE: t("damage.markingDamage"), ROAD_SPILL: t("damage.roadSpill"), UNKNOWN: t("damage.unknown") }
      pieChart.setOption({
        series: [{
...
async function loadSeverity() {
  ...
  try {
    ...
    const items = res.data.data || []
    if (severityChart) {
      const labels: Record<string, string> = { HIGH: t("severity.high"), MEDIUM: t("severity.medium"), LOW: t("severity.low") }
  ...
  try {
    ...
    if (mapTrendChart) {
      ...
      mapTrendChart.setOption({
        xAxis: { data: dates },
        series: [
          { name: t("dash.newMarkers"), data: mapTrend.value.map(d => d.count || 0) },
          { name: t("dash.repaired"), data: mapTrend.value.map(d => d.repairedCount || 0) },
        ],
      })
    }
...
onMounted(async () => {
  ...
  if (mapTrendRef.value) {
    ...
    mapTrendChart.setOption({
      tooltip: { trigger: "axis", backgroundColor: "#fff", borderColor: "#f3f4f6", borderWidth: 1, textStyle: { color: "#111827", fontSize: 11 } },
      legend: { data: [t("dash.newMarkers"), t("dash.repaired")], right: 10, top: 0, textStyle: { color: "#94a3b8", fontSize: 11 }, itemWidth: 12, itemHeight: 8 },
      grid: { left: 40, right: 16, bottom: 28, top: 35 },
      xAxis: { type: "category", data: [], axisLine: { show: false }, axisTick: { show: false }, axisLabel: { color: "#94a3b8", fontSize: 11 } },
      yAxis: { type: "value", splitLine: { lineStyle: { color: "#f1f5f9" } }, axisLabel: { color: "#94a3b8", fontSize: 11 } },
      series: [
        {
          name: t("dash.newMarkers"), type: "line", data: [], smooth: true, symbol: "circle", symbolSize: 6,
        ...
        {
          name: t("dash.repaired"), type: "line", data: [], smooth: true, symbol: "circle", symbolSize: 6,
